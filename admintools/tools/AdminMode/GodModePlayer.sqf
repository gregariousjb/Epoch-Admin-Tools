...
if (EAT_playerGod2) then
{
	...
	if(EAT_logMajorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has god mode",name _player,getPlayerUID _player];
	...
	// Tool use logger
	if(EAT_logMajorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- has DISABLED god mode",name _player,getPlayerUID _player];
		[] spawn {publicVariable "EAT_PVEH_usageLogger";};
	};
	if(!inZone) then {
