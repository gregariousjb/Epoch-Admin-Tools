...
if(enhancedESP2) then { 
	...
	KEY_F8 = (findDisplay 46) displayAddEventHandler ["KeyDown","if ((_this select 1) == 66) then {call F5Menu;};"];
	_player = player;
	// Tool use logger
	if(EAT_logMajorTool) then {
		EAT_PVEH_usageLogger = format["%1 %2 -- enhanced ESP",name _player,getPlayerUID _player];
		...
		*/
		...
		If (AddCrashesToMap && (delayTime == 0 || changed)) then 
		{
			...
			i2 = 0;
			{
				...
				diag_log format ["TEST: Found a %1", _name];
...
if(!enhancedESP2) then 
{
	(findDisplay 46) displayRemoveEventHandler ["KeyDown", KEY_F8];
